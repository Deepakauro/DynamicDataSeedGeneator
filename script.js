<script>
  ...
  function showTips() {
    ...
    for (const prop of properties) {
      ...
      switch (prop.type.toLowerCase()) {
        ...
        case 'guid': note = 'Enter a Guid or "new Guid(...)"'; break;
        case 'string[]': note = 'Pipe-delimited (e.g. Red|Blue|Green)'; break;
        default: note = 'Custom or complex type'; break;
      }
  ...
  function generateCSharpObject(entityName, fieldMap) {
    let csharp = `    new ${entityName} {\n`;

    for (const prop of properties) {
      const key = prop.name;
      const type = prop.type.toLowerCase();
      const raw = (fieldMap[key] || '').trim();
      let line = `        ${key} = `;

      if (type === 'bool' || type === 'boolean') {
        line += raw.toLowerCase() === 'true' ? 'true,' : 'false,';
      } else if (type === 'int' || type === 'int32') {
        const parsed = parseInt(raw);
        line += isNaN(parsed) ? '0,' : `${parsed},`;
      } else if (type === 'guid') {
        if (raw.toLowerCase().startsWith('new guid')) {
          line += `${raw},`;
  ...
  function generateCSharp() {
    ...
    reader.onload = function (e) {
      ...
      for (const row of dataRows) {
        ...
        const objCode = generateCSharpObject(entityName, valueMap);
        csharp += objCode;
      }
